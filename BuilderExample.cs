...
public void buildSmallWorld()
{
    ...
    areaA = new Area("Living Room", "You find yourself in a small, cramped room packed with furniture. There is wooden door to the east leading out into the street");
    areaB = new Area("Street", "This dark, grungy cobblestone street has seen better days. Piles of trash lay heaped about and unidentified darks patches stain the stones.");
    ...
    A1keywordList = new List<string>(keywordArr);

    string[] keywordArr = {"west","door","west door","wood door","wooden door"};
    B1keywordList = new List<string>(keywordArr);

    //Then instantiate the links. Here we are using the default travel and blocked descriptions.
    linkA1 = new Link("A solid looking wooden door.", A1keywordList);
    linkB1 = new Link("A solid looking wooden door.", B1keywordList);
